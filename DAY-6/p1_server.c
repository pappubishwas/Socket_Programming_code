...
int main()
{
    ...
    while (1)
    {
        ...
        // here the code for retrieve student details.....
        ...
        else if (strcmp(buffer, "retrieve") == 0)
        {
            ...
            ssize_t bytes_received = recv(client_fd, username, sizeof(username) - 1, 0);
            if (bytes_received == -1)
            {
            ...
            if (send(client_fd, auth_result, strlen(auth_result), 0) == -1)
            {
                perror("Sending authentication result failed");
                exit(EXIT_FAILURE);
            }
            ...
            {
                char studentRoll[20];
                ssize_t bytes_received = recv(client_fd, studentRoll, sizeof(studentRoll) - 1, 0); // Receive student roll
                if (bytes_received == -1)
                {
                ...
                for (int i = 0; i < numStudents; i++)
                {
                    if (strcmp(studentRoll, students[i].roll) == 0)
                    {
                        ...
                        if (send(client_fd, &students[i], sizeof(StudentInfo), 0) == -1)
                        {
                ...
                if (!studentFound)
                {
                    ...
                    if (send(client_fd, notFoundMessage, strlen(notFoundMessage), 0) == -1)
                    {
